...
\label{def:upper_reducible}
	...
	We say that $F$ is \emph{weakly upper-reducible} if for all $p \in M$ and all $c > F(p)$ there exists a neighborhood $U$ of $p$ in $F_{\leq c}$, a positive constant $\eta > 0$, and a continuous map $\varphi \colon U \times [0,1] \to M$, which satisfies $\varphi(\cdot, 0) = \operatorname{id}_U$, $\varphi((U \cap F_{\leq c-\eta}) \times [0,1]) \subseteq F_{\leq c-\eta}$, and $\varphi(U,1) \subseteq F_{\leq c-\eta}$, such that on %every compact subset $V \subseteq
...
%\cref{thm:sublevel_set_homotopy} can be shown analogously to the argument in \cite[Remark II.6.3]{Struwe.1988}, where level sets of $F$ are assumed to be compact, but $F$ is not assumed to be weakly upper-reducible.
...
\begin{proof}
	...
	Now the natural map from singular to \v{C}ech homology is always surjective for compact metric spaces in dimension 0 \cite{Eda.2000}, so the map $H_{0}(F_{\leq t}) \to \CH_0(F_{\leq t})$ is surjective for all $t$.
...
Morse then goes on to claim that the persistent \v{C}ech homology of this sublevel set filtration is q-tame, provided that $F$ %is bounded from below and
