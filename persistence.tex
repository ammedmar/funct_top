...
\end{equation*}
We say that this filtration is \textit{compact} if all sublevel sets are compact Hausdorff spaces.
...
Of particular importance to us are \v{C}ech homology \cite[Section IX--X]{MR0050886} with coefficients in $\F$, and homology theories in the sense of Eilenberg--Steenrod \cite[Section I]{MR0050886}, such as singular homology, again with coefficients in $\F$ \cite{Eilenberg.1944}.
...
Yet, there are multiple ways to regularize q-tame persistence modules to obtain discrete invariants similar to barcodes \cite{Chazal.2016a, Chazal.2016b, schmahl2020structure}.
...
% When forgetting the endpoints of the intervals in the barcode $\{I_{\lambda}\}_{\lambda \in \Lambda}$ of a persistence module with an interval decomposition, i.e., when disregarding the distinction between open, closed and half-open intervals, we obtain a discrete invariant that is more convenient in many regards, including the formulation of generalized Morse inequalities.
Defined precisely, this invariant known as the \textit{persistence diagram} is the multiset given by the \emph{multiplicity function} $\mathfrak{m} \colon \multiplicityDomain \to \mathbb{N}$ that associates to an element in
\begin{equation*}
\multiplicityDomain =
...
Because information about the endpoints of intervals is irrelevant in the observable category, the persistence diagram associated to the barcode of the radical is still a complete for q-tame modules in the observable category. % and suffices to recover the barcode in many cases of interest.
